...
typedef struct GLNode {
	...
	union {//原子结点和表结点的联合部分
		...
		struct { struct GLNode *hp, *tp; }ptr;//ptr.hp和ptr.tp分别指向表头和表尾
	};
}*GList;//广义表类型
...
{
	int count = 0;
	for (; count<len; count++) *(sub + count) = *(str + start - 1 + count);
...
{//没有分配额外空间，str与hstr都是指针
	int n, i, k;
	Atomtype ch;
	n = strlen(str); i = 0; k = 0;//k记录尚未配对左括号数
	do {
		ch = *(str + i);
		i++;
		if (ch == '(') k++;
...
int CreateGList(GList &L, char *S)//由字符串S创建广义表L,S末尾必须有终结符'\0'
{
	GLNode *p,*q;
	int len;
	if (0 == strcmp(S, "()")) L = NULL; //创建空表
	else {
		if (!(L = (GList)malloc(sizeof(GLNode)))) exit(0);//创建一个新结点
		if (strlen(S) == 1) {
			L->tag = ATOM;
			L->atom = *(S);//创建单原子广义表
		}
			...
			while (1) {
				...
				q = p;
				if (sub) {//表尾不空，在字表后面上添加并列的字表
					if (!(p = (GLNode *)malloc(sizeof(GLNode)))) exit(0);
					p->tag = LIST;
					q->ptr.tp = p;
				}
				else break;
