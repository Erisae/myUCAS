...
class ConvertDist(nn.Module): ## my
    def __init__(self, size):
        super(ConvertDist, self).__init__()  
        self.size = size
    def forward(self, inputs):
        n = inputs.size(0)
        dist = torch.abs(inputs.expand(n, n, self.size).transpose(1, 0) - inputs.expand(n, n, self.size)) # dist[i][j] indicates fi - fj
        out = dist[0].reshape(-1, self.size)
        out.clamp(1e-12)
