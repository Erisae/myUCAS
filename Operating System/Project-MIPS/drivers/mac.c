...
{
    clear_interrupt();
    if (reg_read_32(INT_BASE + INTISR_OFFSET) & (1 << 12)) //网卡中断触发
    {
        int i;
        for (i = 0; i < PNUM; i++)
        {
...
void mac_recv_handle(mac_t *test_mac)
{
    ...
    {
        if (recv_desc[i].tdes0 & 0x80000000) //为1，没有接收
        {
...
uint32_t do_net_recv(uint64_t buf_addr, uint64_t num, uint64_t length)
{
    ...
    while (num > 0)
    {
        ...
        {
            recv_desc[i].tdes0 |= 0x80000000;
            reg_write_32(DMA_BASE_ADDR + 0x8, 1); //要连续写PNUM次才开始收包
        }
        mac_recv_handle(&mac);
        num -= PNUM;
        j++;
    ...
    for (i = 0; i < num;)
    {
        if (!(recv_desc[i].tdes0 & 0x80000000)) //轮询查看是否接收
        {
